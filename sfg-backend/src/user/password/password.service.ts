...
export class PasswordService {
    ...
    async validatePassword(
        ...
        const passwordRecord = await this.passwordRepository.getCurrentPassword(data.userId, prismaClient);
        if (!passwordRecord) {
            throw new NotFoundException(`Password for userId=${data.userId} not found`);
        }
        return await this.comparePasswords(data.rawPassword, passwordRecord.hash);
    };

    private async hashPassword(rawPassword: RawPassword): Promise<PasswordPrisma['hash']> {
        return await bcrypt.hash(rawPassword, this.bcryptSaltRounds);
    }

    private async comparePasswords(rawPassword: RawPassword, hashedPassword: PasswordPrisma['hash']): Promise<boolean> {
